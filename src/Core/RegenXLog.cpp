...
const std::string LOG_PATH = "logs/" + std::format("{:%Y-%m-%d %H:%M:%S}.log", now);

namespace regenx::log {
    bool init() {
        if (!io::file_create(LOG_PATH)) {
    ...
    void info(const std::string& msg) {
        if (io::file_exist(LOG_PATH) && !io::file_write(LOG_PATH, "[INFO] " + msg)) {
    ...
    void debug(const std::string& msg) {
        if (io::file_exist(LOG_PATH) && !io::file_write(LOG_PATH, "[DEBUG] " + msg)) {
    ...
    void trace(const std::string& msg) {
        if (io::file_exist(LOG_PATH) && !io::file_write(LOG_PATH, "[TRACE] " + msg)) {
    ...
    void warning(const std::string& msg) {
        if (io::file_exist(LOG_PATH) && !io::file_write(LOG_PATH, "[WARNING] " + msg)) {
    ...
    void error(const std::string& msg) {
        if (io::file_exist(LOG_PATH) && !io::file_write(LOG_PATH, "[ERROR] " + msg)) {
    ...
    void fatal(const std::string& msg) {
        if (io::file_exist(LOG_PATH) && !io::file_write(LOG_PATH, "[FATAL] " + msg)) {
            std::cerr << "\033[33m[WARNING] Failed to write FATAL log to file\033[0m\n";
        }
        std::cout << "\033[31m[FATAL] " << msg << "\033[0m\n";
        // TODO: abort is for debug and exist for release need to make a flag for it and make it act based on it
        // NOTE: maybe I can make a cleanup function that handles it for now abort is okay
        std::abort();
    }

    void system(const std::string& msg) {
        if (io::file_exist(LOG_PATH) && !io::file_write(LOG_PATH, "[SYSTEM] " + msg)) {
